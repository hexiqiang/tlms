...
class Request {
  ...
  static BaseOptions _options = BaseOptions(
    baseUrl: 'http://192.168.22.16/tlms',
    connectTimeout: 5000,
    receiveTimeout: 3000,
  );
  ...
  static Future<T> _request<T>(String path, {required String method, required Map params, data}) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          if (res['code'] != 0) {
            LogUtil.v(res['code'], tag: '服务器错误，状态码为：');
            EasyLoading.showInfo('服务器错误，状态码为：${res['code']}');
  ...
  Future<T> get<T>(String path, {required Map params}) {
    return _request(path, method: 'get', params: params);
  }

  Future<T> post<T>(String path, {required Map params, data}) {
    print(params);
    print(data);
